...
async function run(configFile, resultPath) {
    ...
    if(typeof configFile === 'undefined') {
        absConfigFile = path.join(__dirname, '..', 'configLoad.json');
    }
    else {
    ...
    for (let j = 0; j < gasLimits.length; j++) {
        for (let i = 0; i < difficulties.length; i++) {
            ...
            while (eachClientRate <= finishRate / nodeCount) {
                ...
                let data = dataAnalysis.processResult(results, eachClientRate, duration);
                ...
                finalResult.throughputBySec.push(data.throughputBySec);
                finalResult.latencyBySec.push(data.latencyBySec);
                eachClientRate *= 2;
            }
            client.stop();
    ...
    fs1.writeFileSync(`resultLoad.json`, tFinalResultStr);    
}
